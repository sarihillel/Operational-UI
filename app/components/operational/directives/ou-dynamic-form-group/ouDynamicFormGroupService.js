'use strict';
//uses: INPUT_TYPES,ui-grid,lodash,ouDateFormat-filter
var ouDynamicFormGroupService = function (INPUT_TYPES, $filter) {
    'ngInject'
   ...
   *****************************************************/
    ...
    *****************************************************/
            ...
            _.forEach(group.fields, function (field) {
                ...
                switch (field.type) {
                    case INPUT_TYPES.GRID:                      
                        ...
                        break;
                    case INPUT_TYPES.LABEL_CONCATENATED:
                        field.value = _setLabelConcatenated(data, field);
                        if (field.IsEditable) {
                            field.popupOptions.onRegisterApi = function (modalApi) {
                                modalApi.on.ok(function (popupData) {
                                    if (data) {
                                        _setDataValues(data, popupData.modalScope.groupsFields);
                                        field.value = _setLabelConcatenated(data, field);
                                    }
                                });
                            }
                            _setGroupsValues(data, field.popupOptions.modalScope.groupsFields);
                        }
                        break;
    ...
    function _setLabelConcatenated(data, field) {
        var labelValue = '';
        var fieldValue = '';
        if (field.values) {
            _.forEach(field.values, function (field) {
                fieldValue = _getProp(data, field.name);
                labelValue += field.charBefore + (fieldValue != 'undefined' ? fieldValue : '') + field.charAfter;
            });
        }
        else {
            var values = _getProp(data, field.name);
            labelValue = _.toString(values);
        }
        return labelValue;
    }
    ...
    function _getFieldValue(field, isText) {
        if (field.type == INPUT_TYPES.SELECT) {
            ...
            if (isText) {
                return _getRangeText($filter('ouDateFormat')(from), $filter('ouDateFormat')(to));
            }
            else
                return {
                    from: from,
...
module.exports = ouDynamicFormGroupService;